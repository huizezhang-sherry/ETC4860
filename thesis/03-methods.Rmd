...
using only the main effect has long been criticised [@martinez2015analysing, @meyer1991misinterpretation, @ottenbacher1991interpretation]
...
- see the calculation of emm when presented with unbalanced data: weight is for averaging over a continuous variable, which is irrelevant for our study -> dont have to adjust weight
...
- `singular.ok = TRUE` need ot specify for `Anova()` when `type = "III"` because the interaciton terms will cause perfect collenarity. Therefore, 2 df is taken from `judge`.
