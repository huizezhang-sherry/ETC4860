...
knitLatex::xTab(format(as.data.frame(lsmean_obj_1), digits = 2), booktabs = TRUE, caption.top = "\\label{tab:result_1} model result")
```
...
- while Gagaler seems to be highly reactive to some cases (i.e. OKS, Nauru_a, Nauru_b). 
...
- Judge Edelman has one in OKS for AU14 being pretty different -> consistent with EDA
...
```{r fig.cap = "THis is the graphical representation of model3_2\\label{fig:model_3_plot2}"}
