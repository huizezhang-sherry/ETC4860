...
```{r goodness-of-fit-1}
pchisq(deviance(binomial_model_1), 
       df.residual(binomial_model_1), lower = FALSE)

library(ResourceSelection)
hoslem.test(binomial_model_1$y, 
            fitted(binomial_model_1), 
            g= 10)
```
...
pchisq(deviance(binomial_model_2), 
       df.residual(binomial_model_2), lower = FALSE)

library(ResourceSelection)
hoslem.test(binomial_model_2$y, 
            fitted(binomial_model_2), 
            g= 10)
```
...
anova(binomial_model_3, test = "Chisq")

int_3 <- confint(emmean_obj_3,  adjust = "bonferroni")
```

```{r goodness-of-fit-3}
pchisq(deviance(binomial_model_3), 
       df.residual(binomial_model_3), lower = FALSE)

library(ResourceSelection)
hoslem.test(binomial_model_3$y, 
            fitted(binomial_model_3), 
            g= 10)
```
...
- Based on the goodness-of-fit test, the p-value of binomial_model_3 is slightly greater than 0.05, indicating the model may be a good fit. 

- However, the hosmer-lemeshow test shows binomial_model_1 has p-value = 1, which indicates model_1 is an adequate fit. other two models has p-value less than 0.05. 
