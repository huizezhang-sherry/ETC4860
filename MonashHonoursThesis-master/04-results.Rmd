...
au_model <- au_tidy %>% 
  ...
  rename(judge = judge, video = video)

binomial_model_1 <- glm(presence ~ judge*AU, 
           family = binomial(link = "logit"),  data = au_model)

glance(binomial_model_1) %>% kable(caption = "\\label{tab:glance_1}summary of model fit")
```
...
```{r fig.cap="plot the multiple comparison result \\label{fig:result_1}"}
ggplot(result_1,aes(x = judge,y = prob,label = .group, col = judge)) +
  geom_point() +
...
<!-- some residual diagnoistic comments here.  -->

<!-- ```{r} -->
<!-- au_model_1_aug <- augment(binomial_model_1, au_model, -->
<!--                           type.predict= "response", -->
<!--                           type.residuals = "response") -->

<!-- # the plot below looks funny because from calculation  presence - .fitted = .resid ->should be two piecewise linear lines -->
<!-- #au_model_1_aug %>% ggplot(aes(x=.fitted, y=.resid)) + geom_point() -->

<!-- #ggplot(au_model_1_aug, aes(x = .resid)) + geom_histogram() -->
...
```{r}
binomial_model_2 <- glm(presence ~ judge*AU + video*AU, 
           family = binomial(link = "logit"),  data = au_model)

lsmean_obj_2 <-  lsmeans(binomial_model_2, ~judge*AU*video)
# result_2 <- multcomp::cld(lsmean_obj_2, alpha = 0.05, type = "response") 
# 
# knitLatex::xTab(format(as.data.frame(result_2), digits = 2), booktabs = TRUE)
```      
...
# binomial_model_2 <- glm(presence ~ judge + AU + video +
#         judge*AU + AU*video + judge*video, 
...
```{r}
binomial_model_3 <- glm(presence ~ judge*AU + video*AU + speaker, 
           family = "binomial",  data = au_model)
