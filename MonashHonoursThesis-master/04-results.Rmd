...
binomial_model_2 <- glm(presence ~ judge*video + AU, 
                        family = binomial(link = "logit"),  
                        data = model_dt_2)
lsmean_obj_2 <-  lsmeans(binomial_model_2, ~judge*video)
result_2 <- multcomp::cld(lsmean_obj_2, alpha = 0.05, type = "response") 

knitLatex::xTab(format(as.data.frame(result_2) %>% filter(!is.na(prob)), digits = 2), booktabs = TRUE, caption.top = "\\label{tab:result_2} model result 2")
```      


```{r fig.cap = "THis is the graphical representation of model2\\label{fig:model_2_plot}"}
type  <- result_2$.group%>% 
  str_trim(side = "both") %>% 
  str_split(pattern = "")

group <- result_2 %>% 
  dplyr::select(video, judge) %>% 
  mutate(type = type) %>%
  unnest() %>% 
  mutate(type = 
           case_when(type == "0" ~ "10",
                     ...
                     TRUE ~ type),
         type = as.numeric(type)) %>% 
  group_by(video, judge) %>% 
  summarise(average_group = mean(type))

result_2 %>% 
  left_join(group, by = c("video", "judge")) %>% 
  ggplot(aes(x = fct_reorder(video,prob),y = prob,
             label = average_group, col = as.factor(average_group))) +
  geom_point() +
  geom_errorbar(aes(ymin = asymp.LCL,ymax = asymp.UCL), width = 0.2) + 
  ggrepel::geom_text_repel(aes(label = average_group)) + 
  facet_wrap(vars(judge), nrow = 2) + 
  theme(legend.position = "none") + 
  coord_flip() + 
  xlab("video")
```
...
binomial_model_3 <- glm(presence ~ judge*speaker + video + AU, 
                        family = "binomial",  data = model_dt_3)

lsmean_obj_3 <-  lsmeans(binomial_model_3, ~judge*speaker)
result_3 <- multcomp::cld(lsmean_obj_3, alpha = 0.05, type = "response") 

knitLatex::xTab(format(as.data.frame(result_3), digits = 2), booktabs = TRUE, caption.top = "\\label{tab:result_3} model result 3")
```
...
group <- result_3 %>% 
  dplyr::select(speaker, judge) %>% 
  mutate(type = type) %>%
  unnest() %>% 
  mutate(type = as.numeric(type)) %>% 
  group_by(speaker, judge) %>% 
  summarise(average_group = mean(type))

result_3 %>% 
  left_join(group, by = c("speaker", "judge")) %>% 
  ggplot(aes(x = fct_reorder(speaker,prob),y = prob,
             label = average_group, col = as.factor(average_group))) +
  geom_point() +
  geom_errorbar(aes(ymin = asymp.LCL,ymax = asymp.UCL), width = 0.2) + 
  ggrepel::geom_text_repel(aes(label = average_group)) + 
