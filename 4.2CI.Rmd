...
au_sim2_majority %>%
 ...
 ggplot(aes(mean_agg, fct_reorder(AU, mean_agg), value)) +
  ...
  ylab("AU") + 
  ggtitle("Simulation of mean percentage appearance using binomial distribution")
...
ci_count %>%
  ...
  facet_wrap(vars(judge_id)) + 
  geom_point(aes(x = mean_ind, col = outliners)) + 
  ylab("AU")
