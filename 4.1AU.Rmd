...
au %>% 
 ...
 select(-suffix) %>% 
 ggplot(aes(x = AU, y = value, col = value, fill = value)) + 
...
aubyframe <- function(judge, video){
  ...
  filter(value == 1, 
         judge_id == judge, 
         video_id == video, 
         AU %in% c("AU01_c","AU02_c", 
                   "AU05_c","AU07_c",
                   "AU14_c","AU15_c",
                   "AU20_c","AU25_c")) %>% 
  ggplot(aes(x = frame_id, y = value, col =speaker)) +
  geom_point() + 
  facet_wrap(vars(AU))
}
...
ActionUnitbySpeaker <- function(dt, judge, case = case_list){
  ...
  if (sum(case_cand %in% sample$video_id) == 0){
    case <- case
  }else{
    case <- intersect(case_cand, sample$video_id)
  }
  
  
  dt %>% 
    filter(AU01_c != "NA", judge_id == judge, video_id %in% case) %>% 
