...
The missings in our case depends on other unobserved variables. For example, the missingness could be due to the fact that a judge is reading the materials on the desk or simply because some faces are not detectable due to the given resolution of the video stream. 


<!-- Because the frames are taken once per minute, we could assume independence among observations, that is, the last observation won't affect the current observation as opposed to the case where the frames are taken once every three seconds.  -->


I think we still have dependences in our data or we will be able to switch between observation from observation - although the autocorrelation of the series is low because we take frame once per minute. 
...
au %>% filter(video_id %in% c("McKell", "OKS", "Parkes")) %>% 
  mutate(AU01_c = ifelse(is.na(AU01_c), 5, AU01_c)) %>% 
  autoplot(AU01_r) + 
  facet_grid(judge_id~ video_id)

au %>% 
  mutate(AU01_r = ifelse(is.na(AU01_r), 5, AU01_r)) %>% 
  autoplot(AU01_r) +
  facet_grid(judge_id~ video_id, scales = "free") + 
  theme(legend.position = "none")

au_subset <- au %>% 
  filter(judge_id == "Keane", video_id == "McKell") 

forecast::na.interp(au_subset$AU01_r) %>% 
