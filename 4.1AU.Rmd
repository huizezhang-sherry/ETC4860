...
au_intensity_all %>% 
  ggplot(aes(x = judge_id, y = intensity, fill = video_id)) + 
  geom_boxplot() + 
  facet_wrap(vars(judge_id), scales = "free_x")
ggsave("images/intensity_boxplot_au.png")

au_intensity_all %>% filter(is_intense ==1) %>% 
  ggplot(aes(x = frame_id, y = intensity, col = speaker)) + 
  geom_point() +
  facet_wrap(vars(judge_id))
ggsave("images/is_intense.png")  
