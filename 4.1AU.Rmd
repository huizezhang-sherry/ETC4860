...
The missing mechanism in our case is a unit-level missing. It depends on other unobserved variables. For example, the missingness could be due to the fact that a judge is reading the materials on the desk so the face is not captured for a particular frame or simply because some faces are not detectable for the given resolution of the video stream. However, since that data is in time series structure, simply drop the missing observation will cause the time interval to be irregular and complicate further analysis. Thus proper imputation method is needed. Because there is no obvious trend or seasonality in each action unit series, imputation method using time series model will not be suitable in our case. Therefore, we choose the linear interpolation from forecast package to impute the missing value for each action unit. Notice that `na.interp` requires at least two non-NA points in a series, we drop 4 video-judge pairs that have `na_proportion =1`, namely (OKS, Gordon), (Parkes, Gordon), (Rinehart_b, Gordon) and (Rinehart_b, Kiefel). 

```{r}
au %>% 
  as_tibble() %>% 
  select(judge_id, frame_id, video_id, AU01_r) %>% 
  mutate(judge_id = as.factor(judge_id)) %>% 
  group_by(video_id, judge_id) %>% 
  summarise(na_count = sum(is.na(AU01_r)), 
            count = n(),
            na_prop = na_count/count, 
            data_prop = 1-na_prop) %>% 
  arrange(-na_prop) # some cases have na_prop ==1 - can't perform linear interp
...
au_imputed <- au %>% 
  ...
  select(-speaker) %>% 
  gather(AU, value, -c(judge_id: video_id)) %>% 
...
ActionUnitbySpeaker <- function(dt, judge, case = case_list){
  ...
  dt %>% 
    ...
    separate(AU_label, c("AU", "suffix"), by = "_") %>%
    select(-suffix) %>%
    ggplot(aes(x = tidytext::reorder_within(AU, by = value,
                                            within = video_id*speaker), 
