...
library(tidyverse)
judges <- c("1","2","3")
names <- map(judges, .f = function(number) {paste0("data/csv/face",number, "/processed")})
filenames <- map(names, list.files, pattern = "*.csv", full.names = TRUE)
names(filenames) <- judges

nauru <- filenames %>% 
  map_df(.f = function(names){
  map_df(names, .f= function(file){
    read_csv(file = file) %>% 
      mutate(info = file)
  })
})
...
```{r eval =FALSE}
library(GGally)
scatmat(nauru_a, columns = 3:5)

ggplot(nauru_a, aes(x=gaze_0_x, y=gaze_0_y, colour=factor(face))) + geom_point() +
  geom_path() + facet_wrap(~face)
ggplot(nauru_a, aes(x=gaze_1_x, y=gaze_1_y, colour=factor(face))) + geom_point() +
  geom_path() + facet_wrap(~face)

ggplot(nauru_a, aes(pose_Tx, y = pose_Ty, colour = factor(face))) + 
  geom_point() + facet_wrap(~face)
library(dplyr)
nauru_a %>% mutate(t=1:423) %>% ggplot(aes(t, pose_Tx, colour = factor(face))) + 
  geom_line() 
library(tidyverse)
head(nauru_a$AU01_r)
...
# Task week 5
- Tidy the data [mostly done] - read something on purrr
